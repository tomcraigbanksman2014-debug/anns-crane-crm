...
type StoredCalculatorState = {
  ...
  craneLines?: unknown;
  labourLines?: unknown;
  equipmentLines?: unknown;
  costLines?: unknown;
  manualAdjustment?: unknown;
  manualAdjustmentLabel?: unknown;
  scope?: unknown;
  notes?: unknown;
};
...
function cleanFallback(value: unknown, fallback = "") {
...
function boolFromAny(value: unknown, fallback = false) {
...
function readQuoteSection(notes: string | null | undefined, label: string) {
  ...
  const labels = [
    ...
    "COST SUMMARY",
    "BREAKDOWN OF CURRENT CHARGES / RATES",
    "ADDITIONAL EQUIPMENT & PERSONNEL",
    "INCLUDED UNDER FULL CPA TERMS",
  ...
  const nextIndexes = labels
    ...
    .map((item) => raw.indexOf(`${item}:`, contentStart))
    .filter((idx) => idx >= 0);

  const end = nextIndexes.length ? Math.min(...nextIndexes) : raw.length;
  return raw.slice(contentStart, end).trim();
}

function parseBreakdownRowsFromNotes(notes: string | null | undefined) {
  const section = readQuoteSection(notes, "BREAKDOWN OF CURRENT CHARGES / RATES");
  if (!section) return [] as Array<{ item: string; amount: string }>;

  return section
    .split(/\n+/g)
    .map((line) => line.trim())
    .map((line) => {
      const match = line.match(/^-?\s*(.*?)\s+[—-]\s*£?([0-9,]+(?:\.[0-9]{1,2})?)/);
      if (!match) return null;
      return {
        item: match[1].replace(/^Job price\s*-\s*/i, "").replace(/^Route\s+\d+\s*-\s*/i, "").trim(),
        amount: match[2].replace(/,/g, ""),
      };
    })
    .filter(Boolean) as Array<{ item: string; amount: string }>;
}

function storedCalculatorStateFromQuote(quote: QuoteRow | null | undefined): StoredCalculatorState | null {
  const sections = objectRecord(quote?.pdf_sections);
  if (!sections) return null;

  const stored = objectRecord(sections.jobCalculator) || objectRecord(sections.job_calculator);
  if (!stored) return null;

  const state = objectRecord(stored.state) || stored;
  return state ? state as StoredCalculatorState : null;
}
...
function hydrateTransportRouteLine(value: unknown, index: number): TransportRouteLine {
  const item = objectRecord(value) || {};
  return newTransportRouteLine({
    id: cleanFallback(item.id, makeId("route")),
    description: cleanFallback(item.description, index === 0 ? "Transport charge" : `Transport charge ${index + 1}`),
    yardPostcode: cleanFallback(item.yardPostcode, "SA10 6JY"),
    collectionPostcode: cleanFallback(item.collectionPostcode),
    viaStops: cleanFallback(item.viaStops),
    deliveryPostcode: cleanFallback(item.deliveryPostcode),
    returnToYard: boolFromAny(item.returnToYard, true),
    chargeableMiles: cleanFallback(item.chargeableMiles),
    ratePerMile: cleanFallback(item.ratePerMile),
    baseRate: cleanFallback(item.baseRate),
    adjustmentPercent: cleanFallback(item.adjustmentPercent),
    actualMiles: numberFromAny(item.actualMiles) || null,
    provider: cleanFallback(item.provider),
    routeMessage: cleanFallback(item.routeMessage),
    routeTone: ["ok", "error", "idle"].includes(cleanFallback(item.routeTone)) ? cleanFallback(item.routeTone) as RouteTone : "idle",
    loading: false,
    quote: boolFromAny(item.quote, true),
  });
}

function hydrateCraneLine(value: unknown, index: number): CraneLine {
  const item = objectRecord(value) || {};
  return newCraneLine({
    id: cleanFallback(item.id, makeId("crane")),
    description: cleanFallback(item.description, index === 0 ? "Crane hire" : `Crane hire ${index + 1}`),
    qty: cleanFallback(item.qty),
    rate: cleanFallback(item.rate),
    adjustmentPercent: cleanFallback(item.adjustmentPercent),
    quote: boolFromAny(item.quote, true),
  });
}

function hydrateLabourLine(value: unknown, index: number): LabourLine {
  const item = objectRecord(value) || {};
  return newLabourLine({
    id: cleanFallback(item.id, makeId("labour")),
    description: cleanFallback(item.description, index === 0 ? "Site team / labour" : `Site team / labour ${index + 1}`),
    men: cleanFallback(item.men),
    days: cleanFallback(item.days, "1"),
    rate: cleanFallback(item.rate),
    adjustmentPercent: cleanFallback(item.adjustmentPercent),
    quote: boolFromAny(item.quote, true),
  });
...
function hydrateEquipmentLine(value: unknown): EquipmentLine {
  const item = objectRecord(value) || {};
  return newEquipmentLine({
    id: cleanFallback(item.id, makeId("equipment")),
    item: cleanFallback(item.item),
    qty: cleanFallback(item.qty),
    rate: cleanFallback(item.rate),
    adjustmentPercent: cleanFallback(item.adjustmentPercent),
    quote: boolFromAny(item.quote, true),
...
function hydrateCostLine(value: unknown, index: number): CostLine {
  const item = objectRecord(value) || {};
  return newCostLine({
    id: cleanFallback(item.id, makeId("cost")),
    item: cleanFallback(item.item, index === 0 ? "Supplier / internal cost" : `Supplier / internal cost ${index + 1}`),
    amount: cleanFallback(item.amount),
    adjustmentPercent: cleanFallback(item.adjustmentPercent),
  });
}
...
export default function JobPackageCalculatorClient({
  ...
  function buildCalculatorState() {
    return {
      ...
      jobDate,
      paymentTerms,
      transportRoutes: transportRoutes.map((route) => ({ ...route, loading: false })),
      craneLines,
      labourLines,
      equipmentLines,
      costLines,
  ...
  function loadCalculatorState(state: StoredCalculatorState) {
    setPackageTitle(cleanFallback(state.packageTitle, "Simple job price"));
    setCustomerId(cleanFallback(state.customerId, customerId));
    setJobDate(cleanFallback(state.jobDate, todayIso).slice(0, 10));
    setPaymentTerms(cleanFallback(state.paymentTerms, "30 days from Month End"));

    const savedRoutes = Array.isArray(state.transportRoutes) ? state.transportRoutes : [];
    setTransportRoutes(savedRoutes.length ? savedRoutes.map(hydrateTransportRouteLine) : [newTransportRouteLine({ id: "route-main" })]);

    const savedCranes = Array.isArray(state.craneLines) ? state.craneLines : [];
    setCraneLines(savedCranes.length ? savedCranes.map(hydrateCraneLine) : [newCraneLine({ id: "crane-main" })]);

    const savedLabour = Array.isArray(state.labourLines) ? state.labourLines : [];
    setLabourLines(savedLabour.length ? savedLabour.map(hydrateLabourLine) : [newLabourLine({ id: "labour-main" })]);

    const savedEquipment = Array.isArray(state.equipmentLines) ? state.equipmentLines : [];
    setEquipmentLines(savedEquipment.length ? savedEquipment.map(hydrateEquipmentLine) : [
      newEquipmentLine({ id: "lifting-beam", item: "Lifting beam" }),
      newEquipmentLine({ id: "crane-mats", item: "Crane mats" }),
      newEquipmentLine({ id: "escort", item: "Escort" }),
      newEquipmentLine({ id: "tracked-carrier", item: "Tracked carrier" }),
    ...
    const savedCosts = Array.isArray(state.costLines) ? state.costLines : [];
    setCostLines(savedCosts.length ? savedCosts.map(hydrateCostLine) : [newCostLine({ id: "supplier-cost" })]);

    setManualAdjustment(cleanFallback(state.manualAdjustment));
    setManualAdjustmentLabel(cleanFallback(state.manualAdjustmentLabel, "Manual adjustment / rounding"));
    setScope(cleanFallback(state.scope, "Crane / transport / lifting works as priced below."));
    setNotes(cleanFallback(state.notes, "Price is subject to clear access, suitable ground conditions, normal working hours unless stated, and no change in scope."));
  }

  function loadQuoteFallback(quote: QuoteRow) {
    setPackageTitle(cleanFallback(quote.subject, "Simple job price"));
    if (quote.client_id) setCustomerId(quote.client_id);
    if (quote.quote_date) setJobDate(cleanFallback(quote.quote_date).slice(0, 10));

    const parsedScope = readQuoteSection(quote.notes, "SCOPE OF WORK");
    const parsedNotes = readQuoteSection(quote.notes, "ADDITIONAL QUOTE NOTES");
    const parsedPaymentTerms = readQuoteSection(quote.notes, "PAYMENT TERMS");

    if (parsedScope) setScope(parsedScope);
    if (parsedNotes) setNotes(parsedNotes);
    if (parsedPaymentTerms) setPaymentTerms(parsedPaymentTerms);

    const breakdownRows = parseBreakdownRowsFromNotes(quote.notes);
    const routeRows = breakdownRows.filter((row) => /transport|route|mileage|delivery|collection/i.test(row.item));
    const craneRows = breakdownRows.filter((row) => /crane|lift|bocker|böcker|jekko/i.test(row.item) && !/beam/i.test(row.item));
    const labourRows = breakdownRows.filter((row) => /team|labour|men|slinger|banksman|supervisor/i.test(row.item));
    const equipmentRows = breakdownRows.filter((row) => !routeRows.includes(row) && !craneRows.includes(row) && !labourRows.includes(row));

    setTransportRoutes(routeRows.length ? routeRows.map((row, index) => newTransportRouteLine({
      id: makeId("route"),
      description: row.item,
      baseRate: row.amount,
      quote: true,
    })) : [newTransportRouteLine({ id: "route-main" })]);

    setCraneLines(craneRows.length ? craneRows.map((row, index) => newCraneLine({
      id: makeId("crane"),
      description: row.item || (index === 0 ? "Crane hire" : `Crane hire ${index + 1}`),
      qty: "1",
      rate: row.amount,
      quote: true,
    })) : [newCraneLine({ id: "crane-main" })]);

    setLabourLines(labourRows.length ? labourRows.map((row, index) => newLabourLine({
      id: makeId("labour"),
      description: row.item || (index === 0 ? "Site team / labour" : `Site team / labour ${index + 1}`),
      men: "1",
      days: "1",
      rate: row.amount,
      quote: true,
    })) : [newLabourLine({ id: "labour-main" })]);

    setEquipmentLines(equipmentRows.length ? equipmentRows.map((row) => newEquipmentLine({
      id: makeId("equipment"),
      item: row.item,
      qty: "1",
      rate: row.amount,
    ...
    setManualAdjustment("");
    setManualAdjustmentLabel("Manual adjustment / rounding");
  }
  ...
  function syncCustomerFromTarget(nextTargetId: string) {
    ...
    if (target === "quote") {
      ...
      if (!quote) return;
      if (quote.client_id) setCustomerId(quote.client_id);
      const savedState = storedCalculatorStateFromQuote(quote);
      if (savedState) {
        loadCalculatorState(savedState);
        setMessage({ tone: "ok", text: "Loaded saved calculator fields from this quote." });
      } else {
        loadQuoteFallback(quote);
        setMessage({ tone: "ok", text: "Loaded the quote details. Older quotes may not have the original exact calculator miles/rates saved." });
      }
    }
  }
    ...
    try {
      const res = await fetch("/api/admin/job-calculator/apply", {
        ...
        body: JSON.stringify({
          ...
          phases,
          lines,
          calculator_state: buildCalculatorState(),
          target_type: target,
