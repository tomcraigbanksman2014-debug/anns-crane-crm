...
export async function POST(
  ...
  try {
    ...
    const storagePath = `${params.id}/lift-plan-specs/${Date.now()}-${safeName(fileName)}`;
    ...
    for (const preview of previewUploads) {
      ...
      const previewPath = `jobs/${params.id}/${documentId}/page-${preview.page_number}.${ext}`;
    ...
    return NextResponse.json(
      { error: error?.message || "Could not prepare lift plan spec sheet upload." },
      { status: 400 }
    );
  }
}
