...
async function resolveOperator() {
  ...
  if (userError || !user) return { supabase, user: null, operator: null, error: "Not signed in" };
  ...
  if (operatorsError) return { supabase, user, operator: null, error: operatorsError.message };

  const operator = (operators ?? []).find((op: any) => matchesOperatorLogin(authEmail, op)) ?? null;
  if (!operator) return { supabase, user, operator: null, error: "No operator record linked to this login." };

  return { supabase, user, operator, error: null };
}

export async function POST(req: Request, { params }: { params: { id: string } }) {
  const { supabase, operator, error } = await resolveOperator();
  if (error || !operator) {
  ...
  if (!endSiteText) {
    return NextResponse.json({ error: "End site is required." }, { status: 400 });
  }
  if (!endPhotoData) {
    return NextResponse.json({ error: "End photo is required." }, { status: 400 });
  }
  if (!endSignatureData) {
    return NextResponse.json({ error: "End signature is required." }, { status: 400 });
  }
  const allowedIssues = ["no_issues", "delay", "safety_issue", "damage", "other"];
  if (!allowedIssues.includes(String(endIssueType ?? ""))) {
    return NextResponse.json({ error: "Select an end of shift outcome." }, { status: 400 });
  }
  if (endIssueType === "other" && !endIssueNotes) {
    return NextResponse.json({ error: "Enter details for Other." }, { status: 400 });
  }
  ...
  if (existingError || !existing) {
    return NextResponse.json({ error: existingError?.message ?? "Shift not found." }, { status: 404 });
  }
