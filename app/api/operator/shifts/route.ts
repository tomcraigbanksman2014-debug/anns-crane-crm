...
export async function POST(req: Request) {
  try {
    const { admin, user, operator, error } = await resolveOperator();
    ...
    if (startSafety.length < 3) {
      return NextResponse.json(
        { error: "Complete all safety checks first." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: e?.message || "Could not start shift." },
      { status: 500 }
    );
  }
}
