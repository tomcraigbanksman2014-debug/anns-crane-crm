...
function serviceGroupForText(value: unknown) {
  ...
  if (
    ...
    text.includes("awkward places where a larger crane")
  ) {
    return "spider";
  }
...
function defaultAvailabilityNoteForService(serviceFocus: string | null) {
  ...
  if (raw.includes("jekko") || raw.includes("spider")) {
    return "Our Jekko / spider crane is ideal for restricted-access lifting, tight sites, internal lifts, glazing, machinery positioning and awkward areas where larger cranes are not practical.";
  }

  if (raw.includes("hk40") || raw.includes("hk 40")) {
    return "Our HK40 is a strong option for city work, restricted sites and projects where a compact truck-mounted crane can keep the job moving efficiently.";
  }

  if (raw.includes("80t") || raw.includes("80 t") || raw.includes("80 tonne") || raw.includes("80 ton") || raw.includes("grove")) {
  ...
  if (
    raw.includes("mobile crane") ||
    raw.includes("100 tonne") ||
    raw.includes("100t") ||
    raw.includes("100 ton") ||
    raw.includes("crane hire")
  ) {
  ...
  if (raw.includes("low loader") || raw.includes("lowloader") || raw.includes("step frame") || raw.includes("haulage")) {
...
function sanitiseAvailabilityNote(note: string | null, serviceFocus: string | null) {
  const serviceGroup = serviceGroupForText(serviceFocus);
  const noteGroup = serviceGroupForText(note);

  if (!note) return defaultAvailabilityNoteForService(serviceFocus);
  if (serviceGroup && noteGroup && serviceGroup !== noteGroup) {
    return defaultAvailabilityNoteForService(serviceFocus);
  }
...
function servicePitch(serviceFocus: string | null) {
  ...
  if (raw.includes("spider") || raw.includes("jekko")) {
    return "We support restricted-access lifting with Jekko / spider cranes, as well as mobile crane hire, contract lifts and practical lifting support for awkward sites.";
  }
...
function buildQuickCampaignDraft(args: {
  ...
  const safeAvailabilityNote = sanitiseAvailabilityNote(availabilityNote, serviceFocus);
  ...
  if (channel === "text") {
    ...
    if (goal === "availability") {
      body = `Hi, Tom from AnnS Crane Hire here. We currently have availability for ${serviceFocus || "mobile crane hire and lifting support"}. ${safeAvailabilityNote ? `${safeAvailabilityNote}. ` : ""}Thought I would let ${companyName(lead)} know in case it helps.`;
    }
  ...
  const hintValues = {
    ...
    availability_note: safeAvailabilityNote,
    custom_cta: customCta,
  };

  const pitch = servicePitch(serviceFocus);
  const lines = [
    `Hi ${contactName(lead)},`,
    "",
    introLine(goal, tone),
  ...
  if (goal === "availability" && safeAvailabilityNote && safeAvailabilityNote !== pitch) {
    lines.push("", safeAvailabilityNote);
  }
  ...
  return {
    subject:
      channel === "email"
        ? interpolate(subjectHint, lead, hintValues) || subjectLine(goal, serviceFocus, safeAvailabilityNote)
        : "",
    body: lines.join("\n"),
    provider: "fallback" as const,
  };
...
export async function POST(
  ...
  try {
    ...
    const forceFallback = goal === "availability" || totalTargets > 25 || purposeSpecificFallbackGoals.includes(goal);
