...
export async function POST(
  ...
  try {
    ...
    const { data: previews } = await admin
      ...
      .eq("vehicle_document_id", params.documentId);

    const previewPaths = (previews ?? [])
      .map((row: any) => String(row.preview_storage_path ?? ""))
      .filter(Boolean);

    await admin.from("asset_document_previews").delete().eq("vehicle_document_id", params.documentId);
    const { error: deleteError } = await admin.from("vehicle_documents").delete().eq("id", params.documentId);
