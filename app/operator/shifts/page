...
export default async function OperatorShiftsPage() {
  ...
  if (userError || !user) redirect("/login?next=/operator/shifts");
  ...
  const { data: rows, error } = await supabase.from("operator_shift_sessions").select("*").eq("operator_id", operator.id).order("started_at", { ascending: false }).limit(60);
  return <ClientShell><div style={{width:"min(900px,95vw)",margin:"0 auto"}}><div style={card}><div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap",alignItems:"center"}}><div><h1 style={{margin:0,fontSize:32}}>Shifts</h1><p style={{marginTop:6,opacity:0.8}}>Shift history for <strong>{operator.full_name}</strong></p></div><a href="/operator/jobs" style={btn}>← Back</a></div>{error ? <div style={errorBox}>{error.message}</div> : !(rows ?? []).length ? <div style={box}>No shifts recorded yet.</div> : <div style={{display:"grid",gap:12,marginTop:18}}>{(rows ?? []).map((row: any) => <div key={row.id} style={box}><div style={{fontWeight:900,fontSize:18}}>{row.status === "started" ? "Active shift" : "Shift complete"}</div><div style={line}><strong>Start:</strong> {fmtDateTime(row.started_at)}</div><div style={line}><strong>End:</strong> {fmtDateTime(row.ended_at)}</div><div style={line}><strong>Start site:</strong> {row.start_site_text ?? "—"}</div><div style={line}><strong>End site:</strong> {row.end_site_text ?? "—"}</div><div style={line}><strong>Issue:</strong> {issueLabel(row.end_issue_type)}</div>{row.end_issue_notes ? <div style={line}><strong>Notes:</strong> {row.end_issue_notes}</div> : null}</div>)}</div>}</div></div></ClientShell>;
}
...
const line: React.CSSProperties = { marginTop:8, fontSize:14 };
