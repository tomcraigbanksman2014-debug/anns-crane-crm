...
export default function TransportLiftPlanForm({
  ...
  const includeTechnicalDrawing = technicalDrawingEnabled(
    form.pack_sections?.include_technical_drawing,
  );
  const approvalErrors = useMemo(
    ...
    [drawingModel, drawingSchedule, form, includeTechnicalDrawing, selectedAccessoryIds.length, technical.errors],
  );
  ...
  return (
    <div style={wrapStyle}>
      ...
      <Section title="Optional technical lift arrangement drawing">
        <label style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          <input
            ...
            disabled={locked}
            onChange={(event) =>
              updateSection("include_technical_drawing", event.target.checked)
            }
          />
          <span>
            ...
            drawing pages but retains any saved drawing data.
          </span>
        </label>
      </Section>
      {includeTechnicalDrawing ? (
      ...
      {includeTechnicalDrawing && drawingValidation.errors.length ? (
