...
export default function TransportLiftPlanForm({
  ...
  async function postForm(payload: TransportLiftPlanData) {
    ...
    if (!res.ok) {
      throw new Error(data?.error || "Error saving transport lift plan.");
    }
    return data;
  }
  ...
  async function generateDraft() {
    ...
    try {
      const res = await fetch(`/api/transport-jobs/${transportJobId}/lift-plan/generate`, {
        method: "POST",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  async function finaliseNow() {
    ...
    try {
      const finalPayload: TransportLiftPlanData = {
        ...form,
  ...
  return (
    <div style={wrapStyle}>
      ...
      <Section title="Job overview">
        <TextAreaField label="Job summary" value={form.job_summary ?? ""} onChange={(v) => update("job_summary", v)} disabled={locked} rows={3} />
        <div style={grid2}>
      ...
      <Section title="Vehicle / HIAB setup">
        ...
        <Field label="Load securing method" value={form.load_securing_method ?? ""} onChange={(v) => update("load_securing_method", v)} disabled={locked} />
      </Section>

      <Section title="Collection / delivery method">
