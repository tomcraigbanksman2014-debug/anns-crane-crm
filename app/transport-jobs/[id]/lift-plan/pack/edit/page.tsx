...
import TransportPackSelectionsForm from "./TransportPackSelectionsForm";
...
export default async function TransportPackEditorPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  return (
    <ClientShell>
      <div style={{ width: "min(1240px, 95vw)", margin: "0 auto", display: "grid", gap: 16 }}>
        <div style={topRow}>
          <div>
            <h1 style={{ margin: 0, fontSize: 32 }}>HIAB pack editor</h1>
            <div style={{ marginTop: 6, opacity: 0.8 }}>
              Transport {(job as any)?.transport_number || "—"} • {client?.company_name || "—"} • {(job as any)?.load_description || "—"}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <a href={`/transport-jobs/${params.id}/lift-plan/pack`} target="_blank" style={secondaryBtn}>
              View full pack
            </a>
          </div>
        </div>
