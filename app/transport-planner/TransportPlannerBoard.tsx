      ...
      {!loading && !error ? (
        <>
          <section style={sectionCard}>
            <div style={sectionTitle}>Unallocated transport jobs</div>

            {(data?.unallocated_jobs ?? []).length === 0 ? (
              <div style={infoBox}>No unallocated transport jobs for this week.</div>
            ) : (
              <div style={{ display: "grid", gap: 10 }}>
                {(data?.unallocated_jobs ?? []).map((item) => (
                  <TransportJobCard key={`unalloc-${item.job_id}`} item={item} href={`/transport-jobs/${item.job_id}`} />
                ))}
              </div>
            )}
          </section>
            ...
            {(data?.vehicles ?? []).map((vehicle) => (
              <section key={vehicle.id} style={sectionCard}>
                ...
                {vehicle.items.length === 0 ? (
                  <div style={infoBox}>No transport allocations this week.</div>
                ) : (
                  <div style={{ display: "grid", gap: 10 }}>
                    {vehicle.items.map((item) => (
                      <TransportJobCard
                        key={`${vehicle.id}-${item.job_id}`}
                        item={item}
...
function TransportJobCard({
  ...
  return (
    <a href={href} style={{ ...jobCardStyle, ...statusTone(item.status) }}>
      ...
      <div style={metaGrid}>
        <Meta label="Dates" value={`${fmtDate(item.transport_date)} → ${fmtDate(item.delivery_date)}`} />
        <Meta label="Times" value={`${item.collection_time ?? "—"} → ${item.delivery_time ?? "—"}`} />
        <Meta label="Operator" value={item.operator_name ?? "—"} />
        <Meta label="Job type" value={item.job_type ?? "—"} />
      </div>
    </a>
  );
}
